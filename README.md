...
So theme may have:
- Presets - a set of theme styles to be enabled by user. This presets my be be complex and drastically modify them look'n'feel. User may choose one option for each preset.
...
- Localizations - Theme may provide a support of locales string. This will be automatically loaded if Playnite's language differ from en_US (en_US locale had to be defined in native theme file to be full functional if theme options extension absent).
...
Extension does not modify any existing theme files. Instead it load additional resources files on start and generate additional resources in memory. Thats if the extension will be disabled - all effects removed. This means that themes have to be designed the way to be fully functional in the 'default' view if extension disabled - all used resources with default values should be declared in scope of native theme files like 'constants.xaml'. And modded resources should override existing ones by theme options extension.

## 'Live' theme options
Since version bindable options and live updates are supported. So instead of referring to xaml resources you may directly bind your theme options using Indexable syntax and PluginSettings Bind:
...
where `ShowHelpMenu` is an example of your variable's or presets constant's name.
...
## Localization
To support localization - no additional theme manifest required. Just add locales resources into Localization folder in your theme and it will be loaded automatically (except of en_US - default theme version has to contain default language in native theme files. I recommend constants.xaml). Please refer to Playnite source code, or almost any (including this) extension code for localization resource samples.
...
Presets option with id equal to 'default' will be shown as selected options in settings UI if nothing specified. Obviously it does not need any Files - as soon as 'Default' theme composition should be done using Playnite build-in theme files only. If no default section is found for preset in options - it will be added automatically with empty preview and files.

Also preset may contain one or many constants redefinition - this may be alternative if it is no reasonable to have dedicated xaml file. Constants definition is next:
```yaml
constant_key:
    Type: constant_type
...
```
Please note that type should not include namespace part - it will be added automatically if required.
...
User will be able to specify values on user settings page.
...
As soon as various themes has very wide customizations of default controls - ability to change styles was introduced with next keys and types. Tune this style according to your theme and settings. The best location is within the `Views\SettingsMenus.xaml` file.
...
Additionally to specify image location withing placeholder use
```xml
...
So if you theme use string variables - you may need to customize look and feel of this 'TextBox button"
